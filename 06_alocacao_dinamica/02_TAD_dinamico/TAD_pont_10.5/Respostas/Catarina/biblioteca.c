...
{
    tBiblioteca *b = malloc (sizeof(*b));
    if (b == NULL) exit (1);

    b->livros = malloc (NUM_INICIAL_LIVROS * sizeof(b->livros));
...
void AdicionarLivroNaBiblioteca(tBiblioteca* biblioteca, tLivros* livro)
{
    if (biblioteca->tamanho >= biblioteca->tam_max_atual)
    {
        ...
        tLivros **newLivro = realloc (biblioteca->livros, biblioteca->tam_max_atual * sizeof(*newLivro));

        biblioteca->livros = newLivro;
    }
